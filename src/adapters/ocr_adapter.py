...
def detect_structured_headings(text: str) -> str:
    ...
    return text
# ───────────────────────── LayoutParser → Markdown directo ─────────────────────────
def layoutparser_markdown_from_image(img_pil: Image.Image) -> str:
    """
    Utiliza layoutparser para segmentar visualmente la imagen y generar Markdown estructurado.
    (Esta función debe implementarse para convertir bloques segmentados en Markdown jerárquico.)
    """
    # Ejemplo de importación local para evitar dependencias globales si no se usa
    import layoutparser as lp
    import numpy as np
    # Modelo PubLayNet detecta: Text, Title, List, Table, Figure
    ...
    layout = model.detect(np.array(img_pil))
    # Ordenar bloques de arriba a abajo
    blocks = sorted(layout, key=lambda b: b.block.y_1)
    md_parts = []
    for block in blocks:
        x1, y1, x2, y2 = map(int, block.coordinates)
        crop = img_pil.crop((x1, y1, x2, y2))
        # OCR sobre cada bloque
        text = pytesseract.image_to_string(crop, lang=OCR_LANG, config=TESSERACT_CONFIG)
        text = _cleanup_text(text)
        # Asignar nivel de encabezado según tipo de bloque
        if block.type == "Title":
            md_parts.append(f"# {text.strip()}")
        elif block.type == "List":
            # Convertir líneas en lista
            items = [f"- {line.strip()}" for line in text.splitlines() if line.strip()]
            md_parts.append("\n".join(items))
        elif block.type == "Table":
            md_parts.append(f"```\n{text.strip()}\n```")
        elif block.type == "Figure":
            md_parts.append(f"![Figura]({x1},{y1},{x2},{y2})")
        else:  # "Text" u otros
            md_parts.append(text.strip())
    return "\n\n".join([part for part in md_parts if part])


# ───────────────────────── Nueva función: extracción visual a Markdown ──────────────
def extract_markdown_with_layoutparser(page: fitz.Page) -> str:
    """
    Aplica layoutparser para segmentar visualmente el contenido de una página PDF y generar Markdown estructurado.

    Args:
        page (fitz.Page): Página PDF.

    Returns:
        str: Markdown generado desde regiones clasificadas.
    """
    pix = page.get_pixmap(dpi=DPI, alpha=False)
    img_pil = Image.open(BytesIO(pix.tobytes("png")))
    img_pil = correct_rotation(img_pil)
    return layoutparser_markdown_from_image(img_pil)