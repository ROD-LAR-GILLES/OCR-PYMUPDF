    ...
    try:
        ...
        if not result["pdf_validity"].get("is_valid", False):
            ...
            if result["pdf_validity"].get("is_encrypted", False):
                print("⚠️ El PDF está encriptado, lo que puede causar problemas")
        ...
        if result["ocr_capability"].get("error"):
            print(f"⚠️ OCR disponible pero con errores: {result['ocr_capability']['error']}")
        else:
            print("✅ OCR está disponible")
            if result["ocr_capability"].get("accuracy", 0) < 50:
                print(f"⚠️ La precisión del OCR es baja: {result['ocr_capability'].get('accuracy', 0):.1f}%")
